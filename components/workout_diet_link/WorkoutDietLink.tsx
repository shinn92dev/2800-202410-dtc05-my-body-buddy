...
const WorkoutDietLink: React.FC<WorkoutDietLinkProps> = ({
    ...
    return (
        <div className="flex justify-center space-x-4 mt-4 relative z-20">
            <a
                href={workoutLink}
                className={`text-center px-4 py-2 rounded hover:bg-gray-200 focus:outline-none ${workoutTextColor} ${workoutUnderline}`}
            >
                {workoutText}
            </a>
            <a
                href={dietLink}
                className={`text-center px-4 py-2 rounded hover:bg-gray-200 focus:outline-none ${dietTextColor} ${dietUnderline}`}
            >
                {dietText}
            </a>
        </div>
